...
router.get("/api/admin/duplicate-conflicts", async (_req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT
        rd.duplicate_group_id AS duplicateGroupId,
        ...
        c.phone AS candidatePhone,
        c.email AS candidateEmail,
        DATE_FORMAT(rd.uploaded_at, '%Y-%m-%d %H:%i:%s') AS submittedAt
      FROM resumes_data rd
      INNER JOIN recruiter r ON r.rid = rd.rid
      LEFT JOIN candidate c ON c.res_id = rd.res_id
      WHERE rd.duplicate_conflict = TRUE
        AND rd.duplicate_group_id IS NOT NULL
      ORDER BY rd.duplicate_group_id, rd.uploaded_at ASC, rd.res_id ASC`,
    );

    const groupsById = new Map();
    rows.forEach((row) => {
      const groupId = String(row.duplicateGroupId || "").trim();
      ...
      groupsById.get(groupId).entries.push({
        resId: row.resId,
        recruiterRid: row.recruiterRid,
        recruiterName: row.recruiterName || row.recruiterRid,
        candidateName: row.candidateName || "Unknown candidate",
        candidatePhone: row.candidatePhone || null,
        candidateEmail: row.candidateEmail || null,
    ...
    return res.status(200).json({ conflicts: Array.from(groupsById.values()) });
  } catch (error) {
    return res.status(500).json({
      message: "Failed to fetch duplicate conflicts.",
      error: error.message,
    });
  }
});

router.post("/api/admin/duplicate-conflicts/:groupId/assign", async (req, res) => {
  const groupId = String(req.params.groupId || "").trim();
  const selectedResId = String(req.body?.resId || "").trim();
  const recruiterRid = String(req.body?.recruiterRid || "").trim();
  if (!groupId || !selectedResId || !recruiterRid) {
    return res.status(400).json({ message: "groupId, resId and recruiterRid are required." });
  }
  ...
  try {
    ...
    const [selectedRows] = await connection.query(
      `SELECT res_id
       FROM resumes_data
       WHERE duplicate_group_id = ? AND res_id = ? AND duplicate_conflict = TRUE
       LIMIT 1`,
      [groupId, selectedResId],
    );
    if (!selectedRows.length) {
      await connection.rollback();
      return res.status(404).json({ message: "Duplicate candidate entry not found." });
    }

    await connection.query(
      `UPDATE resumes_data
       SET duplicate_conflict = FALSE,
           duplicate_hidden = CASE WHEN res_id = ? THEN FALSE ELSE TRUE END,
           rid = CASE WHEN res_id = ? THEN ? ELSE rid END
       WHERE duplicate_group_id = ?`,
      [selectedResId, selectedResId, recruiterRid, groupId],
    );
    await connection.query(
      "UPDATE candidate SET recruiter_rid = ?, rid = ? WHERE res_id = ?",
      [recruiterRid, recruiterRid, selectedResId],
    );
    await connection.query(
      "UPDATE extra_info SET recruiter_rid = ?, rid = ? WHERE res_id = ? OR resume_id = ?",
      [recruiterRid, recruiterRid, selectedResId, selectedResId],
    );
    await connection.commit();
    return res.status(200).json({ message: "Duplicate conflict assigned successfully." });
  } catch (error) {
    await connection.rollback();
    return res.status(500).json({
      message: "Failed to assign duplicate conflict.",
      error: error.message,
    });
  } finally {
    connection.release();
